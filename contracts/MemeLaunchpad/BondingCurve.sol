...
/// @title Bonding Curve Formulas and Protocol Examples
/// @notice Common bonding curve equations and protocols that use them
/**
 * @dev Exponential Bonding Curve
 * Formula: P(x) = a * e^(b * x)
 * Used by: Aragon, some Curation Markets
 */


// We will be using the one used by argon  as shown above other ones are listed below
...
/**
 * @dev Polynomial Bonding Curve
 * Formula: P(x) = a * x^k + b
 * Used by: Giveth, Commons Stack
 */

/**
 * @dev Logarithmic Bonding Curve
 * Formula: P(x) = a * ln(b * x + 1)
 * Used by: Experimental governance/staking models
 */

/**
 * @dev Sigmoid Bonding Curve
 * Formula: P(x) = L / (1 + e^(-k(x - x₀)))
 * Used by: Balancer Boosted Pools, DAO fundraising tools
 */
