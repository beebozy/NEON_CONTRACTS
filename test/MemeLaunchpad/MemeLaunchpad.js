...
describe('Test init', async function () {
    before(async function() {
        ...
        if (ethers.isAddress(BondingCurveAddress)) {
            ...
            BondingCurve = await ethers.deployContract('contracts/MemeLaunchpad/BondingCurve.sol:BondingCurve', [
                // sample Bonding curve config params the constants
                1e15,
                2e15
            ]);
    ...
    //   payer = "FbcyK62bgPP78nLfeVkjVN12hZrYMgk6duwCgPWjMcD6";
      // payer = owner.address; 
    });
    ...
    describe('MemeLaunchpad tests', function() {
        it('createTokenSale', async function () {
            ...
            console.log(" The token address ", Token.target);
    ...
    console.log("Token target", Token.target);
            tx = await MemeLaunchpad.buy(
                Token.target,
              // 10000000 // If WSOL uses 9 decimals like SPL tokens
